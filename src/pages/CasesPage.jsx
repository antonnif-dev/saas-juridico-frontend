...
function CasesPage() {
  ...
  const statusOptions = unique(cases.map(c => c.status));
  const areaOptions = unique(cases.map(c => c.area));
  const urgenciaOptions = unique(cases.map(c => c.urgencia));
  const comarcaOptions = unique(cases.map(c => c.comarca));
  const instanciaOptions = unique(cases.map(c => c.instancia));
  ...
  useEffect(() => {
    ...
    if (filters.comarca !== 'all') result = result.filter(c => c.comarca === filters.comarca);
    if (filters.instancia !== 'all') result = result.filter(c => c.instancia === filters.instancia);

    if (filters.valorAcordadoFaixa !== 'all') {
      result = result.filter(c => {
        const val = Number(c.valorAcordado || 0);
        if (filters.valorAcordadoFaixa === 'Baixo') return val <= 5000;
        if (filters.valorAcordadoFaixa === 'Médio') return val > 5000 && val <= 20000;
        if (filters.valorAcordadoFaixa === 'Alto') return val > 20000;
        return true;
      });
    }
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-slate-50">
      ...
      >
        ...
        <div className="space-y-5 pb-8 md:pb-0">
          ...
          {/* Lista de Filtros */}
          <FilterSelect label="Status" value={filters.status} onChange={(v) => handleFilterChange('status', v)} options={statusOptions} />
          <FilterSelect label="Área" value={filters.area} onChange={(v) => handleFilterChange('area', v)} options={areaOptions} />
          <FilterSelect label="Urgência" value={filters.urgencia} onChange={(v) => handleFilterChange('urgencia', v)} options={urgenciaOptions} />
          <FilterSelect label="Comarca" value={filters.comarca} onChange={(v) => handleFilterChange('comarca', v)} options={comarcaOptions} />
          <FilterSelect label="Instância" value={filters.instancia} onChange={(v) => handleFilterChange('instancia', v)} options={instanciaOptions} />
          <FilterSelect label="Valor acordado" value={filters.valorAcordadoFaixa} onChange={(v) => handleFilterChange('valorAcordadoFaixa', v)} options={['Baixo', 'Médio', 'Alto']} placeholder="Faixa" />
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div>
                      ...
                      <span className="font-medium flex items-center gap-1">
                        <DollarSign className="w-3 h-3" /> {processo.valorAcordado ?? '-'}
                      </span>
                    </div>
                    <div>
                      <span className="text-xs text-slate-400 uppercase block">Comarca</span>
                      <span className="font-medium truncate block">{processo.comarca || '-'}</span>
                    </div>
                    <div>
                      <span className="text-xs text-slate-400 uppercase block">Responsável</span>
                      <span className="font-medium truncate block">
                        {processo.responsavelNome || (processo.responsavelUid ? processo.responsavelUid.slice(0, 8) + '...' : '-')}
                      </span>
                    </div>
                  </div>
                </CardContent>
