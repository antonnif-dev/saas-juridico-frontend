...
function PosAtendimentoPage() {
  ...
  return (<div className="max-w-7xl mx-auto p-4 md:p-8 space-y-8">
    ...
    {isAdmin && (<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div className="md:col-span-2 border-2 border-dashed border-purple-300 bg-purple-50 p-6 rounded-lg flex flex-col sm:flex-row justify-between items-center gap-4">
        <div>
          <div className="flex items-center gap-2 mb-2 text-purple-900">
            <Bot className="w-6 h-6" /> <h3 className="font-bold text-lg">IA Processual</h3>
          </div>
    ...
    <div className="flex flex-col sm:flex-row gap-4 bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
      <div className="relative flex-1">
        ...
        <Input placeholder="Buscar por cliente, número do processo..." className="pl-8" value={search} onChange={(e) =>
          setSearch(e.target.value)} />
      </div>
      <div className="w-full sm:w-64">
        <Select value={faseFilter} onValueChange={setFaseFilter}>
          <SelectTrigger> <SelectValue placeholder="Filtrar por Fase" />
          </SelectTrigger> <SelectContent> <SelectItem value="all">Todas as Fases</SelectItem>
            {fases.map(fase => (
    ...
    {loading ? <p>Carregando processos...</p> : (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6"> {filteredCases.map(processo => (
        <Card key={processo.id} className="hover:shadow-md transition-all">
          <CardHeader className="flex flex-row justify-between items-start pb-2">
            <div> <CardTitle className="text-lg">{processo.titulo}</CardTitle>
              <p className="text-sm font-mono text-slate-500 mt-1"> {processo.numeroProcesso || 'Nº CNJ Pendente'} </p>
            </div>
            <Badge variant="outline" className={getStatusColor(processo.status || 'Aguardando Análise')}> {processo.status || 'Aguardando Análise'} </Badge>
          </CardHeader>
          ...
          <CardFooter className="pt-4 border-t bg-slate-50/50 flex flex-col gap-2">
            <div className="w-full flex justify-between items-center text-xs text-slate-500">
              <div className="flex items-center gap-1">
                ...
                <span>Início: {processo.createdAt?._seconds ? new Date(processo.createdAt._seconds * 1000).toLocaleDateString() : 'Data desc.'}</span>
              </div>
            </div>
            ...
            {processo.status === 'Sentença' && (
              <div className="w-full grid grid-cols-2 gap-2 mt-2">
                <Button variant="outline" size="sm" onClick={() => handleMudarStatus(processo.id, 'Em Recurso')}>
                  <ArrowUpRight className="w-3 h-3 mr-1" /> Iniciar Fase Recursal
                </Button>
                <Button size="sm" className="bg-blue-600 hover:bg-blue-700 text-white" onClick={() => handleMudarStatus(processo.id, 'Trânsito em Julgado')}>
            ...
            {processo.status === 'Em Recurso' && (
              <div className="w-full grid grid-cols-2 gap-2 mt-2">
                <Button size="sm" className="bg-blue-600 hover:bg-blue-700 text-white" onClick={() => handleMudarStatus(processo.id, 'Trânsito em Julgado')}>
                  <Gavel className="w-3 h-3 mr-1" /> Recurso Finalizado (Trânsito)
                </Button>
                <Button size="sm" variant="destructive" onClick={() => handleMudarStatus(processo.id, 'Arquivado')}>
                  <Archive className="w-3 h-3 mr-1" /> Recurso Perdido (Arquivar)
                </Button>
              </div>
            )}
            ...
            {processo.status === 'Em Execução' && (
              <div className="w-full flex justify-between items-center mt-2">
                <span className="text-xs text-green-600 font-bold">Fase de Cobrança Ativa.</span>
                <Button size="sm" variant="destructive" onClick={() => handleMudarStatus(processo.id, 'Arquivado')}>
            ...
            {/* STATUS: ARQUIVADO / CONCLUÍDO (Relatório Final) */}
